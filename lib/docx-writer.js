...
function buildDocumentXml(inventory) {
  ...
  parts.push(paragraph(
    `Automated audit roll-up (axe-core 4.12.1 + IS 17802 custom checks, executed live against every crawled page): ` +
  ...
  const profileLabels = {
    ...
    combined_in: "WCAG 2.1 AA + IS 17802 (Combined India)",
  ...
  parts.push(paragraph("• axe-core 4.12.1 rules mapped to the selected WCAG version's Level A and AA success criteria, executed against the post-hydration DOM of every crawled page (seed + sitemap + hreflang + in-browser-discovered links).", { size: 22 }));
  ...
  parts.push(paragraph("• Cross-framework clause citation on every finding — each WCAG SC is emitted alongside its IS 17802 Part 1 Chapter 9 clause, its EN 301 549 section, and its Section 508 / ADA reference.", { size: 22 }));
  ...
  if (selectedProfile === "is17802") {
    ...
    parts.push(paragraph("• Chapter 7 (Videos with synchronised audio) — partial. Every <video> / <audio> / embedded iframe player (YouTube, Vimeo, Wistia, Dailymotion, JW Player, Kaltura, Brightcove, Facebook Video) is inventoried with its accessible name, captions / audio-description / sign-language track state, autoplay + controls posture, and any nearby transcript link. Missing captions, missing audio description (WCAG 1.2.5), autoplay-with-sound (1.4.2), missing Indian Sign Language interpretation (IS 17802 Ch 7 / WCAG 1.2.6 — detected via <track kind=\"signs\">, srclang=\"sgn-IN\", or an ISL-labelled companion video in the same container), and generic vendor iframe titles (\"YouTube video player\") are emitted as findings. Caption / description / ISL content quality remains a human judgment call.", { size: 22 }));
    parts.push(paragraph("• Chapter 9 (Web content) — full automated pass via axe-core 4.12.1 mapped to WCAG 2.1 A+AA, which IS 17802 Ch 9 mirrors directly.", { size: 22 }));
    parts.push(paragraph("• Chapter 10 (Non-web documents) — link-level discovery + manual-audit plan. Every linked PDF, DOCX / DOC / RTF / ODT, XLSX / XLS / CSV / ODS, PPTX / PPT / ODP is inventoried with link-purpose quality (WCAG 2.4.4), file-type hint, file-size hint, and target=\"_blank\" warning posture. Each linked document also emits an incomplete-review item with the specific Chapter 10 criteria to audit on the document itself (tagged structure, language, title metadata, alt text, reading order, table headers, heading nesting). Same-content multi-format availability (HTML + PDF + DOCX of the same base filename) is recorded as a positive Ch 10 signal. Document-internal audit requires opening each file in a tagged-document checker and is billed separately.", { size: 22 }));
    parts.push(paragraph("• Chapter 12 (Documentation and support) — when the crawl reaches the site's accessibility statement page (detected from URL pattern, page title, or H1), the statement body is validated for: a named conformance standard (WCAG / IS 17802 / EN 301 549 / Section 508 / ADA), a conformance level (A / AA / AAA, full / partial), a last-reviewed or last-updated date, a contact mention, and a reachable feedback mechanism (mailto:, tel:, /contact, /feedback, /grievance, or an embedded form). Missing elements are emitted as IS 17802 Ch 12 findings. If the crawl does not reach any accessibility statement, that itself is recorded.", { size: 22 }));
    parts.push(paragraph("• Chapter 13 (ICT services with relay) — every page is checked for at least one reachable contact channel (mailto:, tel:, /contact, /feedback, /grievance link, or a toll-free 1-800 number in body text). Pages with none are emitted as a review item so the auditor can verify the site's global navigation carries the channel. References to TTY / TRS / National Relay / India Relay / text-relay are surfaced when detected.", { size: 22 }));
    parts.push(paragraph("• Chapters 5 (closed functionality / biometrics), 6 (two-way voice communication), 8 (hardware), and 11 (non-web software) remain out of scope — a web crawler cannot exercise biometric enrolment flows, physical device controls, or native desktop / mobile applications. If any of those are in scope for the engagement they are billed as a separate hardware / native-software audit with manual test cases.", { size: 22 }));
    parts.push(emptyParagraph());
    ...
    parts.push(paragraph("Combined India profile (WCAG 2.1 AA + IS 17802):", { bold: true, size: 22 }));
    parts.push(paragraph("Unions the two India-relevant accessibility standards. Every WCAG finding carries its clause citation under both numbering schemes (WCAG x.y.z / IS 17802 Part 1 Ch 9.x.y.z). Use this profile for Indian government or public-sector engagements that must demonstrate IS 17802 conformance while keeping the WCAG mapping visible.", { size: 22 }));
    parts.push(paragraph("IS 17802 Part 1 chapter coverage in this profile: Chapter 7 partial (video / audio / iframe player inventory with captions, audio description, Indian Sign Language, autoplay, controls — see IS 17802 block above for specifics), Chapter 9 full automated pass, Chapter 10 link-level + manual-audit plan for every linked PDF / Office document, Chapter 12 accessibility-statement content validation when reached, Chapter 13 per-page feedback-channel + relay-service signal detection. Chapters 5, 6, 8 and 11 are out of scope — they require hardware / native-software / biometric test cases that no web crawler can exercise.", { size: 22 }));
    parts.push(emptyParagraph());
  ...
  parts.push(heading("7. Automated Audit Findings", 2));
  {
    ...
    for (const p of pages) {
      ...
      for (const v of (p.audit?.violations || [])) {
        const rid = v.ruleId || v.id || "(unknown)";
        if (!ruleAgg.has(rid)) {
          ruleAgg.set(rid, {
            id: rid, impact: v.impact || "", help: v.help || v.description || "",
  ...
  parts.push(paragraph("• axe-core 4.12.1 is used for automated rule coverage (WCAG 2.1 A + AA tags).", { size: 22 }));
  ...
  parts.push(paragraph("• A VPAT / ACR is produced mapping findings to the requested compliance profile (WCAG 2.1 AA, IS 17802, EN 301 549, Section 508, or ADA Title III).", { size: 22 }));
