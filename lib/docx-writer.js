...
function buildDocumentXml(inventory) {
  ...
  if (selectedProfile === "is17802") {
    ...
    parts.push(paragraph("Unions the three India-relevant standards. Every WCAG finding carries its clause citation under all three numbering schemes (WCAG x.y.z / IS 17802 Part 1 Ch 9.x.y.z / GIGW 5.2.n). Use this profile for Indian government or public-sector engagements that must demonstrate conformance to IS 17802 and GIGW 3.0 simultaneously while keeping the WCAG mapping visible.", { size: 22 }));
    ...
    parts.push(paragraph("GIGW 3.0 governance clauses around grievance redressal SLAs, privacy-policy content approval, and cyber-security certifications remain dependent on client-supplied process evidence and are not auditable automatically.", { size: 22 }));
    parts.push(emptyParagraph());
