...
// via chrome.scripting.executeScript the same way india-checks /
// is17802-checks are.
...
(() => {
  ...
  function deriveRecommendedTests(s) {
    ...
    if (s.pdfLinks > 0) add("PDF links: inform format + size; target PDFs are themselves accessible (tagged)", "WCAG 2.4.4 + separate PDF/UA conformance pass.");
