...
(() => {
  ...
  function collect() {
    ...
    // authoritative URL for this page. Multiple variants (http/https, amp,
    // tracking-tagged, case-different) often point to the same canonical.
    // Used post-crawl as a dedup key so we can collapse server-declared
    // aliases back to one inventory row.
    let canonicalUrl = "";
    try {
