...
//   const blob = await z.finalize("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
//
// finalize() REQUIRES an explicit MIME type. The writer only emits OOXML
// containers (docx/xlsx) that happen to be zip-structured — callers must
// always supply the proper Office MIME so the browser/OS doesn't treat
// the download as a raw .zip archive.
//
...
export function createZip() {
  ...
  async function finalize(mimeType) {
    if (!mimeType || typeof mimeType !== "string") {
      throw new Error("zip-writer: finalize() requires an explicit OOXML MIME type — never emit as application/zip.");
    }
    ...
    return new Blob(all, { type: mimeType });
  }
