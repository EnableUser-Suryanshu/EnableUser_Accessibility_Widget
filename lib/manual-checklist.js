...
// v0.4.6/v0.4.7 — cases the extension machine-assists (visual-checks suite),
// plus cases axe-core or media-checks already covers so the team doesn't
// re-test them blind. "Assisted" = pre-screened, review the eu-* findings
// first then spot-check. "Covered" = an existing automated rule handles the
// programmatic half; the human half (if any) is noted.
export const MACHINE_ASSIST = {
  // — visual-checks suite (eu-* rules) —
  ...
  "K-04": "Partially assisted by eu-positive-tabindex — any tabindex > 0 is flagged automatically; visual-vs-tab-order walk still manual",
  "K-15": "Assisted by eu-focus-stolen-onload — post-load focus position and autofocus attributes checked automatically",
  ...
  "C-08": "Assisted by eu-control-boundary — border/fill contrast computed for every visible enabled form control",
  "C-09": "Partially assisted by eu-focus-ring-contrast — declared :focus outline/box-shadow colours checked against the page background; per-surface verification still manual",
  "C-11": "Assisted by eu-control-boundary — solid-fill buttons included (fill vs adjacent background)",
  "C-15": "Assisted by eu-placeholder-contrast — ::placeholder colour vs field background computed exactly (4.5:1); provable violations",
  "F-13": "Assisted by eu-aria-invalid-onload — fields already aria-invalid at page load are flagged automatically",
  "S-16": "Assisted by eu-duplicate-adjacent-links — adjacent image+text links to the same destination flagged automatically",
  "Z-08": "Assisted by eu-text-spacing-clipped — the 1.4.12 spacing overrides are injected on every page and newly-clipped elements flagged",
  "M-01": "Assisted by eu-motion-no-pause — moving regions sampled for 2.5s; motion with no pause/stop control inside is flagged. Confirm the 5s threshold and control reachability",
  "M-02": "Assisted by eu-motion-no-pause (same sampling pass); keyboard-reachability of the pause control still manual",
  "M-05": "Assisted by eu-motion-no-pause (auto-updating regions caught by the same sampling)",
  "M-06": "Assisted by eu-no-reduced-motion — pages with moving content and no prefers-reduced-motion CSS are flagged",
  // — already covered by axe-core (runs on every page) —
  "F-02": "Programmatic half covered by axe: label / label-title-only — verify the label is VISIBLE and persistent (F-01) by eye",
  "F-06": "Covered by axe: autocomplete-valid (WCAG 1.3.5)",
  "F-07": "Covered by axe: autocomplete-valid — also flags autocomplete='off' misuse patterns; spot-check personal-data fields",
  "S-02": "Programmatic half covered by axe: image-alt / role-img-alt — judging decorative-vs-informative stays human (S-01)",
  "S-03": "Covered by axe: button-name / link-name / aria-command-name — icon-only controls with no accessible name are caught",
  "S-11": "Programmatic half covered by axe: th-has-data-cells / td-headers-attr / scope-attr-valid — verify the VISUAL header relationships match",
  "P-07": "Covered by axe: html-has-lang / html-lang-valid — whether the lang MATCHES the content is human (india-checks also cross-checks scripts)",
  "P-08": "Partially covered by axe: valid-lang + india-checks script detection for Indian languages",
  "P-10": "Covered by axe: duplicate-id-active / duplicate-id-aria",
  "Z-01": "Covered by axe: meta-viewport (user-scalable=no / maximum-scale caught automatically)",
  // — already covered by media-checks (runs on every page) —
  "M-07": "Partially covered by media-checks: autoplaying audio/video detection — the 3-second duration judgment is human",
  "M-09": "Covered by media-checks: autoplay-without-muted is flagged on every crawled page",
  "M-12": "Covered by media-checks: <track kind=captions> declaration checked on every <video> — caption ACCURACY (M-11) stays human",
  "M-13": "Partially covered by media-checks: transcript-link heuristic near audio elements — discoverability judgment stays human"
};
