...
export async function buildReportXlsx(report) {
  ...
  const sheets = [
    ...
    { name: "Inapplicable", rows: objRowsToSheet(CATEGORY_HEADERS, CATEGORY_KEYS, withChecks(report.inapplicableRows)), colWidths: CATEGORY_WIDTHS, opts: { freezeFirstCol: true } },
    {
      name: "Pages",
      rows: objRowsToSheet(
        ["URL", "Title", "Depth", "Source", "Status", "Outcome", "Violations", "Passes", "Incomplete", "Inapplicable", "Template ID", "URL Cluster", "axe Version", "Error"],
        ["url", "page_title", "depth", "source", "status", "outcome", "violations", "passes", "incomplete", "inapplicable", "template_id", "url_cluster", "axe_version", "error"],
        (report.pagesRows || []).map(p => ({ ...p, outcome: pageOutcome(p) }))
      ),
      colWidths: [50, 30, 7, 10, 9, 12, 11, 9, 11, 12, 16, 20, 11, 30],
      opts: { freezeFirstCol: true }
    },
    {
      name: "Templates",
