...
export async function buildReportXlsx(report, { thumbnails = null } = {}) {
  ...
  // Merged-engagement reports (report.coverage present) get a Seen By column
  // right after Page Title: which capture layer saw each issue
  // (crawl | workflow | both). Single-layer workbooks keep their exact
  // pre-merge shape — no empty column noise.
  if (report.coverage) {
    VIOLATIONS_HEADERS.splice(2, 0, "Seen By");
    VIOLATIONS_KEYS.splice(2, 0, "source_layer");
    VIOLATIONS_WIDTHS.splice(2, 0, 10);
  }
  ...
  // Merged engagement — page-coverage map (which layer visited which page)
  // plus the union counts. Present only on MERGE_ENGAGEMENT reports.
  if (report.coverage) {
    const cov = report.coverage;
    const rows = [["Page (normalized URL)", "Seen by"]];
    for (const u of cov.both || []) rows.push([u, "crawl + workflow"]);
    for (const u of cov.crawlOnly || []) rows.push([u, "crawl only"]);
    for (const u of cov.workflowOnly || []) rows.push([u, "workflow only"]);
    rows.push([]);
    const mc = report.mergeCounts || {};
    rows.push([`Coverage: ${(cov.both || []).length} page(s) seen by both layers, ${(cov.crawlOnly || []).length} by the crawl only, ${(cov.workflowOnly || []).length} by the workflow only.`]);
    rows.push([`Unique issues: ${mc.combined ?? "?"} combined — ${mc.both ?? "?"} seen by both, ${mc.crawl ?? "?"} crawl-only, ${mc.workflow ?? "?"} workflow-only. Identity = hash(node html + selector + rule id + normalized page URL), the same recipe both layers use for their own dedup.`]);
    sheets.push({ name: "Coverage", rows, colWidths: [70, 20] });
  }
