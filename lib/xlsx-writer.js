...
// Suppressed Repeats — the audit trail behind the dedup counter. One row
// per suppressed occurrence with the SAME identity hash the kept finding
// carries (nodes' euHash in the Violations/Incomplete data), so the operator
// can join the two and re-verify every suppression by hand.
function buildSuppressedSheet(workflow) {
  const rows = [["Hash", "Rule", "Selector", "Page URL", "Recurred In Step", "Timestamp"]];
  for (const s of workflow.suppressedLog || []) {
    rows.push([s.hash, s.ruleId, s.target, s.pageUrl, s.stepId, s.at]);
  }
  rows.push([]);
  rows.push([`${(workflow.suppressedLog || []).length} suppressed occurrence(s). Identity = hash(node html + selector + rule id + normalized page URL); match Hash against the kept finding's euHash to verify each suppression.`]);
  return rows;
}
...
function buildWorkflowSheet(workflow) {
  ...
  for (const s of workflow.steps || []) {
    // Click steps may carry the control's value and the anchor's target
    // (v0.5.x richer click metadata) — surface both in the Detail column so
    // the workbook reader sees WHAT was clicked and where it led.
    const detail = [
      s.detail || "",
...
export async function buildReportXlsx(report, { thumbnails = null } = {}) {
  ...
  if (report.workflow) {
    ...
    if ((report.workflow.suppressedLog || []).length) {
      sheets.push({
        name: "Suppressed Repeats",
        rows: buildSuppressedSheet(report.workflow),
        colWidths: [18, 26, 40, 50, 14, 22],
        opts: { freezeFirstCol: true }
      });
    }
  }
