  ...
  for (const p of (inventory.pages || [])) {
    ...
    for (const v of (p.violations || p.audit?.violations || [])) {
      ...
      for (const n of nodes) {
        ...
        if (wantThumbs && id && thumbnails.has(id)) {
...
export async function buildReportXlsx(report, { thumbnails = null } = {}) {
  ...
  // Pages sheet thumbnails
  const thumbPositions = [];
  let pageRows = [];
  const pageRowHeights = new Map();

  if (wantThumbs) {
    pageRows.push(["Preview", "URL", "Title", "Depth", "Source", "Status", "Violations", "Passes", "Incomplete", "Inapplicable", "Template ID", "URL Cluster", "axe Version", "Error"]);
    (report.pagesRows || []).forEach((r, idx) => {
      pageRows.push([
        "",
        r.url, r.page_title, r.depth, r.source, r.status, r.violations, r.passes, r.incomplete, r.inapplicable, r.template_id, r.url_cluster, r.axe_version, r.error
      ]);
      const p = report.pages?.[idx];
      const thumb = p?.screenshot?.id ? thumbnails.get(p.screenshot.id) : null;
      if (thumb) {
        thumbPositions.push({
          rowNum: pageRows.length,
          shotId: p.screenshot.id,
          width: thumb.width,
          height: thumb.height
        });
  ...
  // Violations sheet thumbnails
  const violationsThumbPositions = [];
  const violationsRowHeights = new Map();
  let violationsRows = [];

  if (wantThumbs) {
    const vHeaders = ["Preview", "URL", "Page Title", "WCAG", "Level", "SC Name", "IS 17802", "EN 301 549", "Section 508", "ADA", "Rule ID", "Source", "Rule Impact", "Description", "Help", "Tags", "Node Impact", "Target", "XPath", "Ancestry", "HTML", "Failure Summary", "Help URL", "Checks (any/all/none)"];
    const vKeys = ["url", "page_title", "wcag_criterion", "wcag_level", "wcag_name", "is17802_clause", "en301549_clause", "section508_ref", "ada_ref", "rule_id", "rule_source", "rule_impact", "rule_description", "rule_help", "rule_tags", "impact", "selector", "xpath", "ancestry", "html_snippet", "failure_summary", "help_url", "checks_summary"];
    violationsRows.push(vHeaders);

    (withChecks(report.issueRows) || []).forEach(r => {
      const row = [
        "",
        ...vKeys.map(k => {
          const v = r[k];
      ...
      violationsRows.push(row);

      const shotId = r.elementShotId;
      const thumb = shotId ? thumbnails.get(shotId) : null;
      if (thumb) {
        violationsThumbPositions.push({
          rowNum: violationsRows.length,
          shotId: shotId,
          width: thumb.width,
          height: thumb.height
        });
    ...
    for (const t of violationsThumbPositions) {
      const pt = Math.round((t.height + 6) * 0.75);
      violationsRowHeights.set(t.rowNum, Math.max(18, pt));
    }
  } else {
    violationsRows = objRowsToSheet(
  ...
  const sheets = [
    ...
    },
    {
      name: "Violations",
      rows: violationsRows,
      colWidths: wantThumbs
        ? [44, 50, 30, 10, 8, 28, 22, 22, 28, 28, 20, 10, 12, 40, 30, 28, 12, 35, 35, 35, 50, 40, 30, 40]
        : [50, 30, 10, 8, 28, 22, 22, 28, 28, 20, 10, 12, 40, 30, 28, 12, 35, 35, 35, 50, 40, 30, 40],
      opts: {
        freezeFirstCol: true,
    ...
    { name: "Inapplicable", rows: objRowsToSheet(CATEGORY_HEADERS, CATEGORY_KEYS, withChecks(report.inapplicableRows)), colWidths: CATEGORY_WIDTHS, opts: { freezeFirstCol: true } },
    {
      ...
      rows: pageRows,
      colWidths: wantThumbs
        ? [44, 50, 30, 7, 10, 9, 11, 9, 11, 12, 16, 20, 11, 30]
        : [50, 30, 7, 10, 9, 11, 9, 11, 12, 16, 20, 11, 30],
      opts: {
        freezeFirstCol: true,
  ...
  if (wantThumbs && thumbPositions.length > 0 && pagesSheetIdx1 > 0) {
...
export async function buildInventoryXlsx(inventory, { thumbnails = null } = {}) {
  ...
  // Build Violations sheet eagerly so we can capture element screenshot thumbnail row positions.
  ...
  // Each spec = a worksheet carrying embedded images.
  const drawingSpecs = [];
  if (wantThumbs && violationsThumbPositions.length > 0 && violationsSheetIdx1 > 0) {
    drawingSpecs.push({ sheetIdx1: violationsSheetIdx1, positions: violationsThumbPositions });
  }
