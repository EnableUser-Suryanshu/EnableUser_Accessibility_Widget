...
//     date, a contact, and a feedback channel. Here we validate the
//     STATEMENT CONTENT when the current page is the accessibility
...
(function () {
  ...
  const STANDARD_NAMES_RE = /\b(WCAG\s*2\.\d(?:\s*(?:A{1,3}|Level\s*A{1,3}))?|IS\s*17802|EN\s*301\s*549|Section\s*508|ADA)\b/i;
  ...
  function collectStatementContentIssues() {
    ...
    if (!mentionsStandard) {
      rules.push(rule("is17802-statement-missing-standard", {
        ...
        help: "The statement should explicitly name the standard it targets — WCAG 2.1, IS 17802, EN 301 549, Section 508, or ADA. IS 17802 Chapter 12 requires the statement to identify the technical standard and version being claimed.",
        helpUrl: "https://www.w3.org/WAI/planning/statements/",
        tags: ["is17802-ch12", "governance"],
        el: scope,
        failureSummary: "No mention of WCAG, IS 17802, EN 301 549, Section 508, or ADA found in the statement body.",
    ...
    if (!mentionsContact) {
      rules.push(rule("is17802-statement-missing-contact-mention", {
        ...
        help: "Name a team, officer, or inbox that users can reach for accessibility concerns. IS 17802 Ch 12 requires a documented contact route for accessibility issues.",
        helpUrl: "https://www.w3.org/WAI/planning/statements/",
        tags: ["is17802-ch12", "governance"],
        el: scope,
