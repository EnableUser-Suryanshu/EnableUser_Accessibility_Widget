...
(function () {
  // No truncation. These were 300 and 25, which meant a page with 500 links had
  // 200 of them silently unanalysed for 1.4.1, and any rule with more than 25
  // failing nodes reported only the first 25 — with nothing in the report saying
  // findings had been dropped. A truncated audit that looks complete is worse
  // than a slow one: the client fixes 25 issues, re-scans, and 25 more appear.
  //
  // axe-core itself reports every failing node, so capping our own suites was
  // also inconsistent with the rest of the report.
  //
  // Kept as named Infinity rather than deleting the ~16 comparison sites, so the
  // guard structure stays visible and a future cap can be reinstated in one
  // place if a real page ever proves it necessary.
  const MAX_LINKS_ANALYZED = Infinity;
  const MAX_NODES_PER_RULE = Infinity;
  ...
  function checkNoHoverFeedback(styleRules) {
    ...
    // Was anchors.slice(0, 100) — links past the hundredth went unchecked.
    const noHover = [];
    for (const a of anchors) {
  ...
  function checkControlBoundary() {
    ...
    // Was capped at the first 150 visible controls. A long form or a filter-heavy
    // listing page has far more than that, and the ones past 150 were never
    // checked for 1.4.11 boundary contrast.
  ...
  function checkTextSpacing() {
    ...
    const clippedNow = () => {
      const out = new Set();
      // Was capped at the first 600 matching elements. Text-spacing survival is a
      // whole-page property — stopping at 600 meant the foot of a long page was
      // never tested for 1.4.12 clipping.
  ...
  async function checkMotion(styleRules) {
    let candidates = [];
    // Was capped at 10 candidates. Safe to lift: the 2.5s observation window
    // below is a single shared wait, not one per candidate, so the cost of extra
    // candidates is only two cheap getBoundingClientRect snapshots each.
    try { candidates = [...document.querySelectorAll(MOTION_SEL)].filter(el => el.getClientRects().length); } catch {}
