...
export async function navSurfacedCollect() {
  ...
  async function waitStable(timeoutMs = 10000, minCount = 1) {
    ...
    while (Date.now() - start < timeoutMs) {
      const count = document.querySelectorAll(LINK_SEL).length;
      if (count === prev) {
        stableFor += 300;
  ...
  async function expandRevealers() {
    ...
    const targets = [...document.querySelectorAll(EXPANDER_SEL)].slice(0, 60);
    ...
    // Hover menus (mega-menu style)
    const hoverTargets = [...document.querySelectorAll('nav [aria-haspopup], nav [class*="mega" i], nav li[class*="has-submenu" i]')].slice(0, 30);
  ...
  function withinNav(el) {
    ...
    while (cur && cur !== document.body) {
      if (cur.matches && cur.matches(NAV_SEL)) return true;
  ...
  for (const script of document.querySelectorAll(JSON_SEL)) {
