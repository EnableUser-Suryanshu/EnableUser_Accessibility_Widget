...
const BLOCKED_PATH_PREFIXES = [
  ...
  // Generic
  "/cgi-bin",
  // v0.3.1 — SiteScope-discipline additions. All of these are machine /
  // infrastructure endpoints that never render a human-facing page:
  //   • /cdn-cgi    Cloudflare's own endpoints (challenges, zaraz, etc.)
  //   • /.well-known  RFC 8615 / 5785 metadata files (security.txt,
  //                    openid-configuration, acme-challenge, etc.) — always
  //                    machine-readable, never a page.
  //   • /captcha, /bot-check, /verify  anti-bot gates; the crawler hitting
  //     them is a misfire, not content discovery.
  //   • /analytics, /telemetry, /beacon  server-side tracking endpoints
  //     some sites self-host. Always 200 with a 1×1 gif or empty JSON.
  //   • /healthcheck, /health-check, /ping, /pingback  liveness probes.
  //   • /__webpack, /hot-update  dev-mode HMR leftovers left in prod by
...
// Admin-dashboard template demo routes. Keenthemes "Metronic" (and its
// many forks — Start, Demo1-Demo9, Vue/Nuxt/Laravel ports) ships with a
// catalog of demo screens: Account, Widgets, Chat, User Management,
// Ecommerce, Wizards, Error pages, Page Builder, etc. Sites built on
// these templates very commonly leave the sidebar demo links in the
// production nav, so a crawler following <a href> gets sent to all of
// them even though they 404 or render the same error shell. The crawl
// then wastes tab-opens and inflates the inventory with zero-content
// rows (seen on tulsianpms.com: ~20 crafted/apps/error routes each
// returning 0 violations / 0 components / no screenshot).
//
// Prefixes here match Metronic's documented demo route names. If a site
// legitimately uses one of these paths for real content, the operator
// can comment out the specific prefix — we prefer a curated default
// that reflects real-world deployments over blanket permissiveness.
const TEMPLATE_DEMO_PATH_PREFIXES = [
  // Metronic "Crafted" demo catalog — Account, Widgets, Profile, Wizards
  "/crafted",
  // Metronic "Builder" — live page-layout configurator, not a content page
  "/builder",
  // Metronic "Apps" demo subsections. Only Metronic-specific names are
  // blocked; /apps by itself is NOT blocked because many real sites use
  // /apps as a real "applications" index.
  "/apps/chat",
  ...
  "/apps/customers",
  "/apps/calendar",
  // Metronic error-page demos (the real production 404/500 pages on a
  // Metronic site aren't at these URLs — these are the demo gallery
  // showing what each error page looks like)
  "/error/404",
  "/error/500",
  "/error/403",
  "/error/401",
  // Metronic auth demo routes (the demo gallery; real auth pages on a
  // Metronic site are usually at /sign-in, /login, /reset-password, etc.)
  "/authentication/layouts",
  // Metronic pages demo catalog (Profile/Campaigns/Projects/etc.)
  "/pages/profile",
  "/pages/wizards"
];
...
const ARCHIVE_PATH_PREFIXES = [
  ...
  "/attachment",
  "/archive", "/archives",
  // v0.3.1 — SiteScope-discipline additions. Every URL matching these is a
  // derivative view of an already-crawled page:
  //   • /amp          Accelerated-Mobile-Pages light version of an article;
  //                    audit signal identical to the canonical article URL.
  //   • /print        print-view of a page (same body, stripped chrome).
  //   • /trackback    WordPress pingback/trackback sink — HTTP 200 with
  //                    empty shell, not a page.
  //   • /embed        embed-view (e.g., /article/embed/ returns an iframe
  //                    payload for external embedding).
  //   • /feed, /rss,  /atom  RSS / Atom feed endpoints at the path level;
  //                    per-post feeds (/blog/post/feed/) render a comments
  //                    RSS which is not a page. Global feeds are discovered
  //                    separately via FEED_PROBE_PATHS + homepage <link>
  //                    autodiscovery, so blocking them from the anchor
  //                    crawl is lossless.
  //   • /sitemap      XML sitemap endpoints — crawled via walkSitemap, not
  //                    a page to audit.
  "/amp",
  "/print",
  "/trackback",
...
const BLOCKED_QUERY_PARAMS = new Set([
  ...
  // v0.3.1 — SiteScope-discipline additions. Presence-only blocks (value
  // ignored) — any URL carrying these params is a derivative / preview view
  // of an already-crawled canonical page, not a separate content page:
  //   • amp / _amp        AMP variant
  //   • preview / _preview  CMS preview token ("see the draft") — also a
  //                          gate: anonymous crawl will 403 or redirect
  //                          to the published URL anyway.
  //   • version           versioned view of a page (e.g. ?version=2)
  //                          typically used by wikis to show revision
  //                          history; same template, different body.
  "amp", "_amp", "preview", "_preview", "version"
]);
...
export function isCrawlablePath(urlString) {
  ...
  // Metronic / Keenthemes admin-dashboard template demo routes. See
  // TEMPLATE_DEMO_PATH_PREFIXES above for the rationale and the full
  // list of blocked prefixes.
  for (const pre of TEMPLATE_DEMO_PATH_PREFIXES) {
    if (path === pre || path.startsWith(pre + "/") || path.startsWith(pre + "?")) return false;
  }
...
export function canonicalize(raw, base) {
  try {
    ...
    //
    // v0.3.1 — SiteScope-discipline extension. Every ad-platform click-id
    // and common email / social tracker is now stripped so the same article
    // linked from Twitter (?twclid=), Facebook (?fbclid=), LinkedIn
    // (?li_fat_id=), Mailchimp (?mc_cid=), Klaviyo (?_kx=), HubSpot
    // (?_hsenc=), Matomo/Piwik (?pk_campaign=), Adobe (?s_cid=, ?ICID=),
    // Instagram (?igshid=), etc. all canonicalize to the same URL and
    // dedup at the queue.
    //
    // Pattern structure:
    //   • prefix matches end with "_" and match any query key starting with
    //     that prefix (utm_source, utm_medium, pk_campaign, …)
    //   • exact matches end with "$" and match the full key only
    const TRACKING = /^(utm_|fbclid$|gclid$|msclkid$|yclid$|dclid$|twclid$|li_fat_id$|zanpid$|gad_source$|igshid$|clickid$|wbraid$|gbraid$|mc_|_mc_|hsCtaTracking$|_hsenc$|_hsmi$|__hstc$|__hssc$|__hsfp$|_kx$|etid$|pk_|piwik_|matomo_|oly_anon_id$|oly_enc_id$|s_cid$|icid$|trk$|trkCampaign$|trackref$|ref$|ref_src$|ref_url$|refsrc$|referrer$|source$|source_page$|_ga$|_gl$|ga_source$|ga_medium$|ga_campaign$|amp$|_amp_sid$|next$|return$|return_to$|returnurl$|redirect$|redirect_to$|redirectto$|back$|back_url$|backurl$|from$|callback$|origin$|continue$|dest$|destination$)/i;
    const keep = [];
