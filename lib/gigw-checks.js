...
(function () {
  ...
  function nodeFor(el, failure, impact = "moderate") {
    const html = (el && el.outerHTML ? el.outerHTML : "<html>").slice(0, 10000);
  ...
  function checkPdfLinkHints() {
    ...
    return {
      ...
      helpUrl: "https://guidelines.india.gov.in/",
      nodes: bad.slice(0, 100).map(({ a, text, hasTypeHint, hasSizeHint }) => {
        const missing = [!hasTypeHint && "type", !hasSizeHint && "size"].filter(Boolean).join(" + ");
        return nodeFor(a, `Link to .pdf; missing ${missing} hint. Text: "${text.trim().slice(0, 200)}"`, "minor");
      })
    };
  }
